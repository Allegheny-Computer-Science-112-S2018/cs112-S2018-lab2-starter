...
if [ "$CHECK" = true ]; then
  ...
  echo ""
  printf "%s\n" "${blu}Starting to run the mdl checks...${end}"
  echo ""
  mdl README.md
  ...
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/main/java/labtwo/ \
                                     --checkfiles Sentences.java --multicomments 9 --language Java
  determine_exit_code $?
  # --> GatorGrader CHECK: at least 9 "println()" fragment exists in the code
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/main/java/labtwo \
                                     --checkfiles SentencesReverser.java --fragments "println(" --fragmentcounts 5
  determine_exit_code $?
  # --> GatorGrader CHECK: at least 3 "@Test" fragment exists in the code
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/test/java/labtwo \
                                     --checkfiles TestSentences.java --fragments "@Test" --fragmentcounts 3
  determine_exit_code $?
  # --> GatorGrader CHECK: running the program produces exactly 20 lines of output
  python3 gatorgrader/gatorgrader.py --nowelcome --commands "gradle -q --console plain run" --outputlines 26
  determine_exit_code $?
  # --> GatorGrader CHECK: running the program produces the correct output
  python3 gatorgrader/gatorgrader.py --nowelcome --commands "gradle -q --console plain run" --fragments "Tenth"
  determine_exit_code $?
  python3 gatorgrader/gatorgrader.py --nowelcome --commands "gradle -q --console plain run" --fragments "Ninth"
  determine_exit_code $?
  python3 gatorgrader/gatorgrader.py --nowelcome --commands "gradle -q --console plain run" --fragments "Eighth"
  determine_exit_code $?
  ...
  python3 gatorgrader/gatorgrader.py --nowelcome --commits 17
  determine_exit_code $?
 echo ""
